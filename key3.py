import math
st=input()
n=float(input("What is the tendency to return to home? (0 to 1), reccomended is 0.03-0.08"))
home_input = input("Enter home keys for the 8 fingers (e.g., asdfjkl;), or type 'none' for default: ").strip()

# Default home keys: left pinky to right pinky
...
def move(char):
    if char in keys:
        temp_distf = []
        target = keys[char]
        t_x1, t_x2 = target["coords"][0][0], target["coords"][2][0]
        
        for finger in fingers:
            f_data = finger["data"]
            f_x1, f_x2 = f_data["coords"][0][0], f_data["coords"][2][0]
            
            # same row
            if target["row"] == f_data["row"]:
                if target["col"] == f_data["col"]:
                    dist = 0.0
                elif target["col"] > f_data["col"]:
                    dist = t_x1 - f_x2
                else:
                    dist = f_x1 - t_x2       
            # different row, but horizontally overlapping (pure vertical distance)
            elif max(t_x1, f_x1) <= min(t_x2, f_x2):
                if target["coords"][0][1] > f_data["coords"][0][1]:
                    # Target is below finger
                    dist = target["coords"][0][1] - f_data["coords"][1][1]
                else:
                    # Target is above finger
                    dist = f_data["coords"][0][1] - target["coords"][1][1]
            #different row and no horizontal overlap (diagonal corner-to-corner)
            else:
                distances = []
                # If target is to the right, use finger's right corners (2 and 3) 
                # and target's left corners (0 and 1)
                if t_x1 > f_x1:
                    f_corners = [2, 3]
                    t_corners = [0, 1]
                else:
                    # If target is to the left, use finger's left (0 and 1) 
                    # and target's right (2 and 3)
                    f_corners = [0, 1]
                    t_corners = [2, 3]
...
# Home-Row Gravity Penalty: discourage finger drift
            f_center = ((f_data["coords"][0][0] + f_data["coords"][2][0]) / 2, 
                        (f_data["coords"][0][1] + f_data["coords"][1][1]) / 2)
            h_data = finger["home"]
            h_center = ((h_data["coords"][0][0] + h_data["coords"][2][0]) / 2, 
                        (h_data["coords"][0][1] + h_data["coords"][1][1]) / 2)
